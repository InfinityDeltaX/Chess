...
public class Pawn extends Piece{
	...
	public ArrayList<Position> getPossibleMoves(Board b) {
		...
		if(side == Side.WHITE){
			...
			if(p.getPositionRelative(1, 1).doesExistOnBoard() && ((!b.isPositionEmpty(p.getPositionRelative(1, 1))) && b.getPieceAtPosition(p.getPositionRelative(1, 1)).getSide() == Values.getOpposingSide(side))){output.add(p.getPositionRelative(1, 1));} //see if we can move diagonally for a capture. They must have a piece there for this to work!
			if(p.getPositionRelative(-1, 1).doesExistOnBoard() && ((!b.isPositionEmpty(p.getPositionRelative(-1, 1))) && b.getPieceAtPosition(p.getPositionRelative(-1, 1)).getSide() == Values.getOpposingSide(side))){output.add(p.getPositionRelative(-1, 1));} //check diagonally the other way
		}
